...
class ListPengalamanActivity : AppCompatActivity() {

    private lateinit var dbHelper: DatabaseHelper
    private lateinit var adapter: PengalamanAdapter
    private lateinit var rvPengalaman: RecyclerView
    ...
    private var listData: List<Pengalaman> = emptyList()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_list_pengalaman)
        ...
        rvPengalaman = findViewById(R.id.rv_pengalaman)
        tvEmpty = findViewById(R.id.tv_empty)
        etSearch = findViewById(R.id.et_search)
        tvCountBadge = findViewById(R.id.tv_pengalaman_count_badge)
        ...
        rvPengalaman.layoutManager = LinearLayoutManager(this)
        adapter = PengalamanAdapter(
            emptyList(),
            onItemClick = { id ->
                val intent = Intent(this, DetailPengalamanActivity::class.java)
                intent.putExtra("pengalaman_id", id)
                AppUtils.navigateTo(this, intent)
            },
            onEdit = { id ->
                val intent = Intent(this, TambahPengalamanActivity::class.java)
        ...
        bottomNav.selectedItemId = R.id.navigation_experience
        bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.navigation_experience -> true
    ...
    private fun loadData() {
        if (userEmail.isNotEmpty()) {
            listData = dbHelper.getAllPengalaman(userEmail)
            adapter.updateList(listData)
            tvCountBadge.text = "✦ ${listData.size} Pengalaman Terekam"

            if (listData.isEmpty()) {
                tvEmpty.visibility = View.VISIBLE
                rvPengalaman.visibility = View.GONE
            } else {
                tvEmpty.visibility = View.GONE
                rvPengalaman.visibility = View.VISIBLE
            }
        }
    }
    ...
    private fun filterList(query: String) {
        val filtered = listData.filter {
            ...
            it.organisasi.contains(query, ignoreCase = true) ||
            it.posisi.contains(query, ignoreCase = true) ||
    ...
    private fun showDeleteDialog(id: Int) {
        AlertDialog.Builder(this)
            ...
            .setMessage("Apakah Anda yakin ingin menghapus pengalaman ini?")
            .setPositiveButton("Hapus") { _, _ ->
                val result = dbHelper.deletePengalaman(id)
                if (result > 0) {
